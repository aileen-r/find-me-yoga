<script>
	...
	import { durationQuestion } from '../../data/questions';
	...
	$: buttonsOutYCoord =
		activeQuestion === questions.length && direction === directions.forward ? 0 : 30;
...
<div class="flex flex-col flex-auto items-center justify-center w-full h-full text-center">
	{#if activeQuestion === durationQuestion.id}
		<div
			class="absolute text-center w-full -mt-24"
			in:fly|local={{
				delay: direction === directions.back ? 600 : 0,
				duration: 600,
				y: getYCoord(durationQuestion.id, true)
			}}
			out:fly|local={{ duration: 600, y: getYCoord(durationQuestion.id, false) }}
		>
	...
	{#if activeQuestion > 1}
		...
		>
			...
			<button class="btn btn-primary" on:click={handleCtaClick} disabled={ctaDisabled}
				>Find me yoga</button
			>
		</div>
	{/if}
